...
CONTACT_ATTR_LIST = ("first_name", "last_name", "email", "phone")
...
class RequestAdmin(CrmModelAdmin):
    ...
    fieldsets = [
        ...
        (
            ...
            {
                "classes": ("collapse",),
                "fields": ["subsequent", "case", ("modified_by", "ticket")],
            },
        ),
    ]
    ...
    def change_view(self, request, object_id, form_url="", extra_context=None):
        ...
        extra_context["emails"] = self.get_latest_emails("request_id", object_id)
        extra_context["has_change_deal_perm"] = request.user.has_perm("crm.change_deal")
    ...
    def get_form(self, request, obj=None, **kwargs):
        ...
        if request.method == "POST" and "_create-deal" in request.POST:
            ...
            works_globally = Department.objects.get(id=department_id).works_globally
            if works_globally:
    ...
    def response_post_save_change(self, request, obj):
        if "_view_print_version" in request.POST:
            return HttpResponseRedirect(reverse("print_request", args=(obj.pk,)))
    ...
    def save_model(self, request, obj, form, change):
        ...
        if (
            ...
            or "duplicate" in form.changed_data
            and obj.duplicate
        ):
            obj.pending = False
    ...
    def save_related(self, request, form, formsets, change):
        ...
        if "_create-deal" in request.POST:
            if not any((obj.pending, obj.deal, obj.duplicate)):
                ...
                CrmEmail.objects.filter(ticket=obj.ticket, deal__isnull=True).update(
                    deal=d
                )
                if obj.deal:
                    _notify_deal_owners(request, obj)
    ...
    def counterparty(self, obj):
        ...
        if counterparty:
            ...
            return mark_safe(f'<a href="{url}">{counterparty.full_name}</a>')
        return obj.company_name

    @staticmethod
    @admin.display(description=mark_safe(table_loyalty_icon))
    def loyalty(obj):
        if obj.subsequent:
            return mark_safe(loyalty_icon.format(subsequent_title))
        return mark_safe(primary_icon.format(primary_title))
    ...
    @admin.display(description=subject_safe_icon, ordering="request_for")
    ...
    @admin.display(description=today_safe_icon, ordering="receipt_date")
...
def check_for_counterparty_assignment(request: WSGIRequest, object_id: int) -> None:
    ...
    if object_id:
        ...
        if counterparty_assigned_to:
            ...
            if tag:
                username = (
                    get_user_model().objects.get(id=counterparty_assigned_to).username
                )
                msg = _("Found the counterparty assigned to")
                messages.add_message(request, tag, f"{msg} {username}")


def copy_attrs(obj: Request, attr: str, attr_list) -> None:
    for a in attr_list:
        if not getattr(obj, a, None):
            setattr(obj, a, getattr(getattr(obj, attr), a, None))
...
def _change_related_objs_attrs(obj: Request, attrs: list) -> None:
    ...
    emails = CrmEmail.objects.filter(request=obj, incoming=True, inquiry=True)
...
def _get_or_create_deal(obj: Request, request: WSGIRequest) -> Deal:
    try:
        deal = Deal.objects.get(request=obj, ticket=obj.ticket)
    except Deal.DoesNotExist:
        date = get_formatted_short_date()
        msg = _("Request")
        department_id = get_department_id(obj.owner)
        stage = Stage.objects.filter(department_id=department_id, default=True).first()
        deal = Deal(
        ...
        if request.user.department_id:  # NOQA
            ...
            deal.currency_id = Currency.objects.get(is_state_currency=True).id
        if obj.contact:
        ...
        for product in obj.products.all():
            Output.objects.create(deal=deal, product=product, quantity=1)
        ...
        messages.success(request, format_html(msg, **msg_dict))
    return deal
...
def _notify_deal_owners(request: WSGIRequest, obj: Request) -> None:
    ...
    if request.user != deal.owner:
        notify_user(
            deal, deal.owner, DEAL_OWNER_NOTICE, DEAL_OWNER_NOTICE, request=request
        )
    if deal.co_owner and request.user != deal.co_owner:
        notify_user(
            deal,
            deal.co_owner,
            DEAL_CO_OWNER_NOTICE,
            DEAL_CO_OWNER_NOTICE,
            request=request,
        )
...
def _update_request_email(obj: Request) -> None:
    try:
        ...
        request_email = CrmEmail.objects.get(
            request=obj, incoming=True, trash=False, inquiry=True
        )
        ...
        if save_request_email:
            request_email.save(update_fields=["owner", "deal", "lead"])
